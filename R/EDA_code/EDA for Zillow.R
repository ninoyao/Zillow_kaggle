...
library(leaflet)
library(lubridate)
properties <- fread('/Users/nino/Desktop/Kaggle/Zillow/properties_2016.csv')
transactions <- fread('/Users/nino/Desktop/Kaggle/Zillow/train_2016_v2.csv')
...
##Difference in log(Zestimata)-log(Saleprice)  positive loggeror means overestimating 
##loggerror
transactions %>% 
...
##Absolute logerror
##How close Zestimate to Saleprice
...
## Correlation with absolute logerror - num_features
...
##Correlation with absolute logerror - area_features
...
##Correlation with absolute logerror - tax_ features
...
##Correlation with logerror - num_features -There seems to be small negative correlations between the num features and logerror.
vars <- good_features$feature[str_detect(good_features$feature,'num_')]
...
corrplot(cor(tmp, use="complete.obs"),type="lower")
## Correlation with logerror - area_ features
vars <- good_features$feature[str_detect(good_features$feature,'area_')]

tmp <- cor_tmp %>%  select(one_of(c(vars,"logerror")))

corrplot(cor(tmp, use="complete.obs"), type="lower")
## Correlation with logerror - tax_features
vars <- setdiff(good_features$feature[str_detect(good_features$feature,'tax_')],c("tax_delinquency","tax_year"))
...
leaflet(tmp) %>% 
  ...
  addMiniMap()
##Map absolute logerror - Show the absolute logerror on map. Red = higher.
